...
def _read_images(ps_image_filenames_list):
    ...
    for i, each_image in tqdm(enumerate(ps_image_filenames_list, 1)):
        ...
        # each_image_class = each_image.split("/")[1]
...
def save_sun_data(save_tfrecords=False, save_type='tf'):
    attr_dim = cfg.SUN_ATT_DIM
    class_num = cfg.SUN_CLASS_NUM
    image_num = cfg.SUN_IMAGE_NUM
    ss_split_path = os.path.join(cfg.SS_SPLIT_PATH, 'SUN')
    ps_split_path = os.path.join(cfg.PS_SPLIT_PATH, 'SUN')
    save_data_path = checkdir(os.path.join(cfg.PREPROCESSED_DATA_PATH, 'SUN'))
    image_path = os.path.join(cfg.SUN_PATH, 'images')

    data_path = cfg.SUN_PATH
    ...
    ps_image_filenames_list = convert_mat_to_list(ps_image_filenames, 'sun')
...
def read_sun_data(filename_queue, batch_size, image_mean, mode):
    attr_dim = cfg.SUN_ATT_DIM

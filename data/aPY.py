...
def _read_class(ps_attr_data):
    ...
    allclasses_names_list = convert_mat_to_list(allclasses_names, 'sun', convert_type='origin')
    class2index = {}
    index2class = {}
...
def _read_images(ps_image_filenames_list, index2class, ps_labels):
    index2image = {}
    image2index = {}
    all_images = {}
    class2images = {}
    ...
    for i, each_image in tqdm(enumerate(ps_image_filenames_list, 1)):
        # multiple training sample with the same image path
        if not each_image in image2index:
            image2index[each_image] = [] 
        image2index[each_image].append(i)

        index2image[i] = each_image

        each_label = ps_labels[i-1, 0]
        each_image_class = index2class[each_label]
        if not each_image_class in class2images:
            class2images[each_image_class] = []
        class2images[each_image_class].append(each_image)
        # the same images used in multiple classes
        if not each_image in all_images:
            all_images[each_image] = {}
        all_images[each_image][i] = {'index': i, 'class': each_image_class}
...
def save_apy_data(save_tfrecords=False, save_type='tf'):
    attr_dim = cfg.APY_ATT_DIM
    class_num = cfg.APY_CLASS_NUM
    image_num = cfg.APY_IMAGE_NUM
    ss_split_path = os.path.join(cfg.SS_SPLIT_PATH, 'APY')
    ps_split_path = os.path.join(cfg.PS_SPLIT_PATH, 'APY')
    save_data_path = checkdir(os.path.join(cfg.PREPROCESSED_DATA_PATH, 'APY'))
    image_path = cfg.APY_PATH
    data_path = cfg.APY_PATH
    ...
    ps_labels = io.loadmat(checkfile(os.path.join(ps_split_path, 'ps_labels.mat')))['labels']
    ps_image_filenames_list = convert_mat_to_list(ps_image_filenames, 'apy')

    class2index, index2class = _read_class(ps_attr_data)
    class2images, allimages, index2image, image2index =  _read_images(ps_image_filenames_list, index2class, ps_labels)
    ...
    def _get_imageinfo(mat_index):
        ...
        for i in mat_index:
            ...
            each_image = {}
            each_image['index'] = i
            each_image['path'] = index2image[i]
            assert each_image['path'] == ps_image_path
            each_image['class'] = class2index[allimages[each_image['path']][i]['class']] # start from 1
...
def read_apy_data(filename_queue, batch_size, image_mean, mode):
    attr_dim = cfg.APY_ATT_DIM
    ...
    if mode == 'train':
        _index, _class, _pretrain_class, _class_attr, _image_feat, _comp_image_feat, _image \
    ...
    return _index, _class, _pretrain_class, _class_attr, None, _image_feat, _comp_image_feat, _image